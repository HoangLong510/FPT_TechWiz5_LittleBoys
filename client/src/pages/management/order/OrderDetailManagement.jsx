...
import { fetchOrderDetailManagementApi, updateOrderTypeApi } from './service'
...
export default function OrderDetailManagement() {
    ...
    const handleFetchOrderDetail = async () => {
        setLoading(true)
        try {
            const res = await fetchOrderDetailManagementApi(id)
            if (res.success) {
                setOrder(res.order)
            }
        } catch (error) {
    ...
    const handleUpdateOrderType = async (type) => {
        const data = {
            id,
            type
        }
        const res = await updateOrderTypeApi(data)
                    ...
                    <>
                        <Grid container spacing={3}>
                            ...
                            <Grid item xs={12} md={6}>
                                <Box>
                                    <Typography variant="h6" gutterBottom sx={{ display: 'flex', alignItems: 'center' }}>
                                        <LocalShipping sx={{ mr: 2 }} /> Order Summary
                                    </Typography>
                                    <Divider sx={{ my: 2 }} />
                                    <Grid container spacing={2}>
                                        <Grid item xs={6}>
                                        ...
                                        <>
                                            <Box sx={{ mt: 3 }}>
                                                <Button
                                                    ...
                                                    color="primary"
                                                    sx={{ mr: 2 }}
                                                    onClick={() => { handleUpdateOrderType('success') }}
                                                >
                                                    Complete Order
                                                </Button>
                                                <Button variant="outlined" color="error" onClick={() => { handleUpdateOrderType('canceled') }}>
                                                    Cancel Order
                                                </Button>
                                            </Box>
                                        </>
