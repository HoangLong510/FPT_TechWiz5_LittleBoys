...
export default function CategoryDetail() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      dispatch(
        setPopup({ type: "error", message: "Failed to update category." })
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <Helmet>
        <title>
          {import.meta.env.VITE_PROJECT_NAME} | {t("CategoryDetail")}
        </title>
      </Helmet>
      <Box
        sx={{
      ...
      >
        ...
        >
          <Box sx={{ fontWeight: "bold" }}>{t("CategoryDetail")}</Box>
          <Link to="/management/categories">
            <Button>{t("View all categories")}</Button>
          </Link>
        </Box>
